...
Use Conventional Commits, matching repository history: `feat: add ...`, `fix: prevent ...`, or `refactor: simplify ...`. Keep the summary imperative, lowercase, and under 72 characters. Develop features and fixes on focused branches based on `master`; Since the project is simple, direct commit to the master branch, don't create new branch/PRs, do not force-push. Commit message should explain user-visible CLI changes, data or migration impact, and validation performed. Link the relevant issue in commit description when available. Screenshots are unnecessary for this CLI; include concise terminal examples when output or arguments change.
